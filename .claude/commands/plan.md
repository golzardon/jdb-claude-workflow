...
- Read `.agents/decisions/` for past decisions on related topics
- Read recent files in `.agents/plans/` for related work
- Check `.claude/rules/architecture.md` for system constraints
...
- Check CLAUDE.md and `.claude/rules/` for project-specific rules and conventions

**3. Dependency Analysis**

- Catalog external libraries relevant to feature
- Understand how libraries are integrated (check imports, configs)
- Find relevant documentation in project docs and CLAUDE.md
...
After the plan file is saved, confirm to the user:
- "Plan saved to `.agents/plans/{name}.md`. Run `/execute .agents/plans/{name}.md` to implement, then `/commit` when done."

**Important:** Do NOT commit the plan file. It stays as an untracked local artifact in `.agents/plans/` (gitignored).
